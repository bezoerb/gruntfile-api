...
(function(root, factory) {
    ...
    /**
     * Get type for registered task (ArrayExpression|FunctionExpression)
     *
     * @param identifier
     * @returns {Object|*|Mixed}
     */
    function getRegisteredTaskType(identifier) {
        var type = traversal.reduce(function(acc,node) {
            if (node && isRegisterTaskBody(this, identifier)){
                 acc = _.last(node).type;
            }
            return acc;
        });
        return type;
    ...
    function registerTask(identifier, task, options) {
        var params = _.assign({append: true, prepend: false, overwrite: false}, options),
            ...
            type = getRegisteredTaskType(identifier),
            code,codeInt, syntax, condition;


        traversal.forEach(function(node) {

            // task is not registered yet. Add new
            ...
            // Task definition is an array and task is array. Prepend, append, or overwrite based on options.
