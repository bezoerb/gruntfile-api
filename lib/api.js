...
(function(root, factory) {
    ...
    function isRegisterTaskArguments(traversalNode, identifier) {
        function check(node) {
            return node.parent && node.parent.parent && node.parent.parent.parent &&
                isGruntMainBody(node.parent.parent.parent) &&
                node.parent.node.callee && node.parent.node.callee.property &&
                node.parent.node.callee.property.name === 'registerTask' &&  node.key === 'arguments' &&
                _.isArray(node.node) && _.first(node.node).value && _.first(node.node).value === identifier;
        }
        ...
        return traversalNode.key === 'elements' && traversalNode.parent &&
            ...
            check(traversalNode.parent.parent);
    }
     ...
     * @param {array|function} tasks    Subtasks or functions to call
     ...
     * @returns {*}
     */
    function registerTask(identifier, tasks, options) {
        var taskDefinition = convertToString(tasks),
            params = _.assign({append:true,prepend:false,overwrite:false},options),
            code = 'grunt.registerTask(\'' + identifier + '\', ' + taskDefinition + ');',
            syntax = getAstFromCode(code);

        traversal.forEach(function(node) {
            var registered = hasTaskRegistered(identifier);

            // task is not registered yet. Add new
            if (isGruntMainBody(this) && !registered) {
                this.update(node.concat(syntax));

            // task with the same name ist registered. Prepend, append, or overwrite based on options.
            } else if (isRegisterTaskArguments(this,identifier) && registered) {
                var code = 'var tmp = ' + taskDefinition,
                    codeAst = getAstFromCode(code),
                    syntax = traverse(codeAst).get(['0', 'declarations', '0', 'init','elements']);
